...
def split_dataset(data):
	...
	train, test = data[0:2000], data[510:606]
...
def forecast(model, history, n_input):
	...
	# global x
	# x = str(x)
	# np.savetxt('np'+ x +'.csv',input_x,header="Power (Kw)")
	# x = x+1
...
# load the new file
dataset = read_csv('BMS_Final.csv', header=0, infer_datetime_format=True, parse_dates=['datetime'], index_col=['datetime'])
# split into train and test
train, test = split_dataset(dataset.values)
# evaluate model and get scores
n_input = 4
score, scores, actual, predictions = evaluate_model(train, test, n_input)
...
# SAVE array and predictions
import numpy
numpy.save('actual.npy', actual)
